...
async function main() {
  ...
  if (isDev) {
    ...
    for (const { title, description, price, tags } of jobs.slice(0, 5))
      await account0.job.addJob(title, description, ethers.parseEther(price.toString()), tags)

    for (const { title, description, price, tags } of jobs.slice(5, 10))
      await account1.job.addJob(title, description, ethers.parseEther(price.toString()), tags)
