...
contract ChatManager {
  ...
  function openChannel(uint _escrowId) public isEscrowParty(_escrowId) {
  ...
  function closeChannel(uint _escrowId) public isEscrowParty(_escrowId) {
  ...
  ) public isEscrowParty(_escrowId) {
  ...
  modifier isEscrowParty(uint _escrowId) {
    require(
      escrowManager.isEscrowEntity(_escrowId),
      "You're not allowed to send a message in this Channel."
    );
    _;
  }
}
